...
contract VRFCoordinator is VRF, VRFRequestIDBase {
    ...
    HotspotOracleTokenInterface internal HSO;
    BlockHashStoreInterface internal blockHashStore;

    constructor(address _HSO, address _blockHashStore) public {
    ...
    struct Callback {
        ...
        // Amount of HSO paid at request time. Total HSO = 1e9 * 1e18 < 2^96, so
    ...
    struct ServiceAgreement {
        ...
        uint96 fee; // Minimum payment for oracle response. Total HSO=1e9*1e18<2^96
        bytes32 jobID; // ID of corresponding HotspotOracle job in oracle's DB
    }
     ...
     * @param _fee minimum HSO payment required to serve randomness
     * @param _oracle the address of the HotspotOracle node with the proving key and job
     * @param _publicProvingKey public key used to prove randomness
     * @param _jobID ID of the corresponding HotspotOracle job in the oracle's db
     */
    function registerProvingKey(
        ...
        require(
            _fee <= 1e9 ether,
            "you can't charge more than all the HSO in the world, greedy"
        );
    ...
    /**
     * @notice Called by HSO.transferAndCall, on successful HSO transfer
     *
     ...
     *
     * @param _sender address: who sent the HSO (must be a contract)
     * @param _fee amount of HSO sent
    ...
    /**
     * @notice creates the HotspotOracle request for randomness
     *
     ...
     * @param _feePaid Amount of HSO sent with request. Must exceed fee for key
        ...
        )
    {
        ...
        uint256 actualSeed =
            uint256(keccak256(abi.encodePacked(preSeed, blockHash)));
    ...
    /**
     * @dev Allows the oracle operator to withdraw their HSO
     * @param _recipient is the address the funds will be sent to
     * @param _amount is the amount of HSO transferred from the Coordinator contract
     */
        ...
        hasAvailableFunds(_amount)
    {
        ...
        assert(HSO.transfer(_recipient, _amount));
    }
     ...
     */
    modifier sufficientHSO(uint256 _feePaid, bytes32 _keyHash) {
        require(
    ...
    /**
     * @dev Reverts if not sent from the HSO token
     */
    modifier onlyHSO() {
        require(msg.sender == address(HSO), "Must use HSO token");
        _;
    }

    ...
    HotspotOracleTokenReceiver
{
    ...
    /**
     * @notice Deploy the contract with a specified address for the HSO
     * and Oracle contract addresses
     * @dev Sets the storage for the specified addresses
     * @param _HSO The address of the HSO token contract
     */
    constructor(address _HSO) public {
        if (_HSO == address(0)) {
            setPublicHotspotOracleToken();
        } else {
    ...
    function getHotspotOracleToken() public view override returns (address) {
        return HotspotOracleTokenAddress();
    }
    ...
    function oracleRequest(
        ...
        if (_payment > totalPayment) {
            ...
            HotspotOracleTokenInterface _HSO =
                HotspotOracleTokenInterface(HotspotOracleTokenAddress());
            assert(_HSO.transfer(_sender, overage));
        }
    }

    /**
     * @dev Creates HotspotOracle requests to each oracle in the service agreement with the
    ...
    function createRequests(
        ...
        HotspotOracle.Request memory request;
        ...
        for (uint256 i = 0; i < sa.oracles.length; i++) {
            request = buildHotspotOracleRequest(
                sa.jobIds[i],
                address(this),
                this.HotspotOracleCallback.selector
            );
            request.setBuffer(_data);
            outgoingRequestId = sendHotspotOracleRequestTo(
                sa.oracles[i],
                request,
                sa.payments[i]
            );
     ...
     * @dev The recordHotspotOracleFulfillment protects this function from being called
     * by anyone other than the oracle address that the request was sent to
     * @param _requestId The ID that was generated for the request
     * @param _data The answer provided by the oracle
     */
    function HotspotOracleCallback(bytes32 _requestId, int256 _data)
        external
    ...
    /**
     * @notice Allows the owner to withdraw any HSO balance on the contract
     * @dev The only valid case for there to be remaining HSO on this contract
     * is if a user accidentally sent HSO directly to this contract's address.
     */
    function withdrawHSO() external onlyOwner {
        HotspotOracleTokenInterface _HSO =
            HotspotOracleTokenInterface(HotspotOracleTokenAddress());
        require(
            _HSO.transfer(msg.sender, _HSO.balanceOf(address(this))),
            "Unable to transfer"
        );
    }
    ...
    function cancelOracleRequest(
        ...
        HotspotOracleTokenInterface _HSO =
            HotspotOracleTokenInterface(HotspotOracleTokenAddress());
        require(_HSO.transfer(req.sender, _payment), "Unable to transfer");
    }
    ...
    modifier checkCallbackAddress(address _to) {
        require(
            _to != HotspotOracleTokenAddress(),
            "Cannot callback to HSO"
        );
        _;
    }
}
